...
#include "associate_mesh.h"
  ...
  ! 1D
  call my_def_var(ncid, 'zbar',              NF_DOUBLE, 1, (/nl_id /),    zbar_id,              &
   'depth of levels'                       )
  call my_def_var(ncid, 'Z',                 NF_DOUBLE, 1, (/nl1_id/),    z_id,                 &
   'depth of layers'                       )
  call my_def_var(ncid, 'elem_area',         NF_DOUBLE, 1, (/elem_n_id/), elem_area_id,         &
   'element areas'                         )
  call my_def_var(ncid, 'nlevels_nod2D',     NF_INT,    1, (/nod_n_id/),  nlevels_nod2D_id,     &
   'number of levels below nodes'          )
  call my_def_var(ncid, 'nlevels',           NF_INT,    1, (/elem_n_id/), nlevels_id,           &
   'number of levels below elements'       )
  call my_def_var(ncid, 'nod_in_elem2D_num', NF_INT,    1, (/nod_n_id/),  nod_in_elem2D_num_id, &
   'number of elements containing the node')
  call my_def_var(ncid, 'nod_part',          NF_INT,    1, (/nod_n_id/),  nod_part_id,          &
   'nodal partitioning at the cold start'  )
  call my_def_var(ncid, 'elem_part',         NF_INT,    1, (/elem_n_id/), elem_part_id,         &
   'element partitioning at the cold start')
  call my_def_var(ncid, 'zbar_e_bottom',     NF_DOUBLE, 1, (/elem_n_id/), zbar_e_bot_id,        &
   'element bottom depth')
  call my_def_var(ncid, 'zbar_n_bottom',     NF_DOUBLE, 1, (/nod_n_id/) , zbar_n_bot_id,        &
   'nodal bottom depth')
  ! 2D
  call my_def_var(ncid, 'nod_area',        NF_DOUBLE, 2, (/nod_n_id, nl_id/), nod_area_id,        &
   'nodal areas'                 )
  call my_def_var(ncid, 'elem',            NF_INT,    2, (/elem_n_id, id_3/), elem_id,            &
   'elements'                    )
  call my_def_var(ncid, 'nodes',           NF_DOUBLE, 2, (/nod_n_id,  id_2/), nod_id,             &
   'nodal geo. coordinates'      )
  call my_def_var(ncid, 'nod_in_elem2D',   NF_INT,    2, (/nod_n_id, id_N/),  nod_in_elem2D_id,   &
   'elements containing the node')
  call my_def_var(ncid, 'edges',           NF_INT,    2, (/edge_n_id, id_2/), edges_id,           &
   'edges'                       )
  call my_def_var(ncid, 'edge_tri',        NF_INT,    2, (/edge_n_id, id_2/), edge_tri_id,        &
   'edge triangles'              )
  call my_def_var(ncid, 'edge_cross_dxdy', NF_DOUBLE, 2, (/edge_n_id, id_4/), edge_cross_dxdy_id, &
   'edge cross distancess'       )
  call my_def_var(ncid, 'gradient_sca_x',  NF_DOUBLE, 2, (/elem_n_id, id_3/), gradient_sca_x_id,  &
   'x component of a gradient at nodes of an element')
  call my_def_var(ncid, 'gradient_sca_y',  NF_DOUBLE, 2, (/elem_n_id, id_3/), gradient_sca_y_id,  &
