...
#include "associate_mesh.h"
    ...
    call transfer_mesh(nod_elem2D_gpu, nod_in_elem2D, my_size * size(nod_in_elem2D, 1))
    call transfer_mesh(nod_num_elem2D_gpu, nod_in_elem2D_num, my_size)
    ...
    call alloc_var(fct_adf_h_gpu, fct_adf_h, 2 * my_size * (nl - 1))
    ...
    call alloc_var(fct_minus_gpu, fct_minus, my_size * (nl - 1))
#endif
...
#include "associate_mesh.h"
    ...
    call fct_ale_pre_comm_acc2(  alg_state, fct_ttf_max_gpu, fct_ttf_min_gpu, fct_plus_gpu, fct_minus_gpu,&
                                ...
                                nlevs_nod2D_gpu, nlevs_elem2D_gpu, elem2D_nodes_gpu, nod_num_elem2D_gpu, nod_in_elem2D,&
    ...
    alg_state = 0
    ! Insert call to second C-function here
    if(alg_state < 1) then
        !___________________________________________________________________________
        ! b3. Limiting   
        !Vertical
    ...
    alg_state = 0
    ! Insert call to third C-function here
    if(alg_state < 1) then
        !Horizontal
        do edge=1, myDim_edge2D
            enodes(1:2)=edges(:,edge)
    ...
    if (iter_yn) then
        alg_state = 0
        ! Insert call to 4th C-function here
        if (alg_state < 1) then
            !___________________________________________________________________________
            ! c. Update the LO
            ! Vertical
    ...
    alg_state = 0
    ! Insert call to 5th C-function here
    if (alg_state < 1) then
        !___________________________________________________________________________
